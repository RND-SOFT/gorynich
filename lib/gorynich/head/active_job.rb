    ...
    module ActiveJob
      ...
      included do
        ...
        around_perform do |job, block|
          @current_domain ||= Gorynich::Current.domain
          @current_tenant ||= Gorynich::Current.tenant
          Gorynich.with(job.current_tenant, domain: job.current_domain) do |current|
            ::ActiveJob::Base.logger.tagged("Tenant(#{current.tenant})") do
              ::ActiveJob::Base.logger.debug do
                current_database = ::ActiveRecord::Base.connection.execute(
                  'SELECT current_database();'
                ).first['current_database']
